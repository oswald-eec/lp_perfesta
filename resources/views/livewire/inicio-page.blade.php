{{-- PÁGINA DE INICIO PERFESTA --}}
<div class="bg-white text-gray-800 font-[Montserrat Alternates]" x-data="{ slide: 0, slides: ['actividad1.jpg', 'actividad2.jpg', 'actividad3.jpg'] }" x-init="setInterval(() => slide = (slide + 1) % slides.length, 5000)">
    ...
    {{-- CARDS DE CATEGORÍAS --}}
    <section class="py-24 bg-gradient-to-b from-white via-green-50 to-white font-[Montserrat Alternates]">
                ...
                @foreach($categorias as $item)
                    ...
                    <div class="relative group bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-2xl border border-green-100 transition-all duration-500 ease-out hover:-translate-y-2">
                        ...
                        <div class="hidden md:flex absolute inset-0 items-center justify-center bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                            <a href="{{ $item['ruta'] }}"
                            class="bg-white text-green-700 font-semibold px-8 py-3 rounded-full shadow-lg hover:bg-green-50 transition">
                                Ver {{ $item['titulo'] }}
                            </a>
                        </div>
                    </div>
    ...
    <section id="testimonios" class="relative bg-green-900 py-24 px-6 md:px-16 text-center overflow-hidden text-white">
        <div class="max-w-6xl mx-auto relative z-10">
            {{-- Título y subtítulo --}}
            <h2 class="text-4xl md:text-5xl font-[Montserrat Alternates] font-extrabold text-yellow-400 mb-3">
                Lo que dicen nuestros clientes
            </h2>
            <p class="text-green-100 mb-14 text-lg">
                Miles de personas ya disfrutan <span class="text-yellow-300 font-semibold">Perfesta</span>
            </p>
            ...
            <div class="grid gap-10 md:grid-cols-2 lg:grid-cols-3">
                @foreach ($testimonios as $item)
                    <div 
                        class="bg-white/10 backdrop-blur-md shadow-lg rounded-2xl p-8 border border-white/10 
                            hover:shadow-yellow-500/20 hover:-translate-y-1 transition-all duration-500 ease-out 
                            animate-fadeInUp">
                        {{-- Estrellas --}}
                        ...
                        {{-- Texto --}}
                        <p class="text-green-50 italic mb-6 leading-relaxed">
                            “{{ $item['texto'] }}”
                        </p>
        ...
        <div class="absolute top-0 left-0 w-80 h-80 bg-yellow-400/10 rounded-full blur-3xl opacity-30 -translate-x-32 -translate-y-20"></div>
        <div class="absolute bottom-0 right-0 w-[28rem] h-[28rem] bg-green-400/20 rounded-full blur-3xl opacity-20 translate-x-20 translate-y-24"></div>
    ...
    {{-- CONTACTO --}}
        <section id="contacto" class="relative bg-gradient-to-r from-green-700 via-green-600 to-[#FF6F61] text-white py-24 px-6 md:px-16 overflow-hidden font-[Montserrat Alternates]">
            <div class="max-w-6xl mx-auto grid md:grid-cols-2 gap-14 items-center relative z-10">
                ...
                <div>
                    ...
                    <ul class="space-y-5 text-green-50">
                        <li class="flex items-center gap-3">
                            ...
                            <span class="text-base">+591 79327294</span>
                        </li>
                    ...
                    class="inline-flex items-center gap-3 mt-10 bg-yellow-400 text-green-900 px-7 py-3 rounded-full font-semibold shadow-lg hover:bg-yellow-300 hover:scale-105 hover:shadow-[0_0_20px_rgba(255,255,255,0.3)] active:scale-95 transition-all duration-300">
                        {{-- Icono de WhatsApp --}}
                ...
                <div class="bg-white/10 backdrop-blur-md rounded-2xl p-10 border border-white/20 shadow-lg hover:shadow-2xl transition duration-300">
                    <h3 class="text-white text-2xl font-semibold mb-6">Haz tu pedido</h3>

                    <form wire:submit.prevent="suscribir" class="space-y-5">
                        <input type="text" placeholder="Nombre completo" 
                            class="w-full p-3.5 rounded-md bg-white/20 placeholder-white/70 text-white border  focus:bg-white/30 transition">
                        ...
                        <textarea rows="3" placeholder="¿Qué cócteles te interesan?" 
                            class="w-full p-3.5 rounded-md bg-white/20 placeholder-white/70 text-white border  focus:bg-white/30 transition"></textarea>

                        <button type="submit"
                            class="w-full bg-white text-green-800 font-semibold py-3 rounded-md hover:bg-green-50 active:scale-95 transition-all duration-300">
                            Enviar Solicitud
                        </button>
                    </form>
                </div>
